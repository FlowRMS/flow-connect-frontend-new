import { __module as W } from "./revogrid-pro298.js";
var B = W.exports, Z;
function Y() {
  return Z ? W.exports : (Z = 1, (function(K, F) {
    (function(h, u) {
      K.exports = u();
    })(B, function() {
      var h = (function(u, f) {
        ...
        o.fn = o.prototype, o._init = function(t) {
          ...
          if (i(c)) {
            ...
            return c.forEach(function(b, R) {
              o.arange(t.length).forEach(function(k) {
                g[k][R] = t[k][b];
              });
            }), g;
          }
          for (var y = new Array(t.length), _ = 0; _ < t.length; _++)
            y[_] = [t[_][c]];
          return y;
        }, o.cola = function(t, c) {
          ...
          var c = [], g, y, _, b, R;
          for (i(t[0]) || (t = [t]), y = t.length, _ = t[0].length, R = 0; R < _; R++) {
            for (g = new Array(y), b = 0; b < y; b++)
              g[b] = t[b][R];
            c.push(g);
          }
          ...
          var y, _, b, R, k;
          for (i(t[0]) || (t = [t]), _ = t.length, b = t[0].length, R = g ? t : new Array(_), y = 0; y < _; y++)
            for (R[y] || (R[y] = new Array(b)), k = 0; k < b; k++)
              R[y][k] = c(t[y][k], y, k);
          return R.length === 1 ? R[0] : R;
        }, o.cumreduce = function(t, c, g) {
          var y, _, b, R, k;
          for (i(t[0]) || (t = [t]), _ = t.length, b = t[0].length, R = g ? t : new Array(_), y = 0; y < _; y++)
            for (R[y] || (R[y] = new Array(b)), b > 0 && (R[y][0] = t[y][0]), k = 1; k < b; k++)
              R[y][k] = c(R[y][k - 1], t[y][k]);
          return R.length === 1 ? R[0] : R;
        }, o.alter = function(t, c) {
          return o.map(t, c, !0);
        }, o.create = function(t, c, g) {
          var y = new Array(t), _, b;
          for (r(c) && (g = c, c = t), _ = 0; _ < t; _++)
            for (y[_] = new Array(c), b = 0; b < c; b++)
              y[_][b] = g(_, b);
          return y;
        };
        function s() {
        ...
        o.identity = function(t, c) {
          ...
          r(y) || (y = !1);
          var _ = [], b = d(t, c), R = (c * b - t * b) / ((g - 1) * b), k = t, z;
          for (z = 0; k <= c && z < g; z++, k = (t * b + R * b * z) / b)
            _.push(y ? y(k, z) : k);
          return _;
          ...
          function a(c, g, y, _) {
            ...
            if (g === f && y === f && _ === f)
              return o.copy(c);
            if (g = g || 0, y = y || c.length, g = g >= 0 ? g : k + g, y = y >= 0 ? y : k + y, _ = _ || 1, g === y || _ === 0)
              return [];
            if (g < y && _ < 0)
              return [];
            if (g > y && _ > 0)
          ...
          function t(c, g) {
            ...
            if (g = g || {}, n(g.row)) {
              ...
              var b = o.rowa(c, g.row);
              return y = g.col || {}, a(b, y.start, y.end, y.step);
            }
            if (n(g.col)) {
              var R = o.cola(c, g.col);
              return _ = g.row || {}, a(R, _.start, _.end, _.step);
            }
            _ = g.row || {}, y = g.col || {};
            var k = a(c, _.start, _.end, _.step);
          ...
          if (n(c.row)) {
            ...
            var b = c.row;
            return y.forEach(function(k, z) {
              t[b][k] = g[z];
            }), t;
          }
          if (n(c.col)) {
            c.row = c.row || {}, c.row.start = c.row.start || 0, c.row.end = c.row.end || t.length, c.row.step = c.row.step || 1, _ = o.arange(
              c.row.start,
          ...
          return g[0].length === f && (g = [g]), c.row.start = c.row.start || 0, c.row.end = c.row.end || t.length, c.row.step = c.row.step || 1, c.col.start = c.col.start || 0, c.col.end = c.col.end || t[0].length, c.col.step = c.col.step || 1, _ = o.arange(
            c.row.start,
      ...
      return (function(u, f) {
        ...
        u.gammaln = function(p) {
          ...
          var i = 1e-8, r = w - 1, n = d - 1, e = 0, o, s, l, v, m, a, t, c, g, y, _;
          if (p <= 0)
            return 0;
          if (p >= 1)
            return 1;
          for (w >= 1 && d >= 1 ? (l = p < 0.5 ? p : 1 - p, v = f.sqrt(-2 * f.log(l)), t = (2.30753 + v * 0.27061) / (1 + v * (0.99229 + v * 0.04481)) - v, p < 0.5 && (t = -t), c = (t * t - 3) / 6, g = 2 / (1 / (2 * w - 1) + 1 / (2 * d - 1)), y = t * f.sqrt(c + g) / g - (1 / (2 * d - 1) - 1 / (2 * w - 1)) * (c + 5 / 6 - 2 / (3 * g)), t = w / (w + d * f.exp(2 * y))) : (o = f.log(w / (w + d)), s = f.log(d / (w + d)), v = f.exp(w * o) / w, m = f.exp(d * s) / d, y = v + m, p < v / y ? t = f.pow(w * y * p, 1 / w) : t = 1 - f.pow(d * y * (1 - p), 1 / d)), _ = -u.gammaln(w) - u.gammaln(d) + u.gammaln(w + d); e < 10; e++) {
            if (t === 0 || t === 1)
              return t;
            if (a = u.ibeta(t, w, d) - p, v = f.exp(r * f.log(t) + n * f.log(1 - t) + _), m = a / v, t -= v = m / (1 - 0.5 * f.min(1, m * (r / t - n / (1 - t)))), t <= 0 && (t = 0.5 * (t + v)), t >= 1 && (t = 0.5 * (t + v + 1)), f.abs(v) < i * t && e > 0)
              break;
          }
          return t;
        ...
        function w(i, r, n) {
          ...
          if (_ >= m)
            return 1;
          var b = 2 * u.normal.cdf(_, 0, 1, 1, 0) - 1;
          b >= f.exp(l / n) ? b = f.pow(b, n) : b = 0;
          var R;
          i > a ? R = t : R = c;
          for (var k = _, z = (m - _) / R, I = k + z, E = 0, T = n - 1, H = 1; H <= R; H++) {
            for (var Q = 0, x = 0.5 * (I + k), G = 0.5 * (I - k), A = 1; A <= e; A++) {
              var L, O;
              o < A ? (L = e - A + 1, O = g[L - 1]) : (L = A, O = -g[L - 1]);
              var V = G * O, P = x + V, N = P * P;
              if (N > v)
                break;
              var J = 2 * u.normal.cdf(P, 0, 1, 1, 0), C = 2 * u.normal.cdf(P, i, 1, 1, 0), U = J * 0.5 - C * 0.5;
              U >= f.exp(s / T) && (U = y[L - 1] * f.exp(-(0.5 * N)) * f.pow(U, T), Q += U);
            }
            Q *= 2 * G * n / f.sqrt(2 * f.PI), E += Q, k = I, I += z;
          }
          return b += E, b <= f.exp(s / r) ? 0 : (b = f.pow(b, r), b >= 1 ? 1 : b);
        }
        function d(i, r, n) {
          var e = 0.322232421088, o = 0.099348462606, s = -1, l = 0.588581570495, v = -0.342242088547, m = 0.531103462366, a = -0.204231210125, t = 0.10353775285, c = -453642210148e-16, g = 0.0038560700634, y = 0.8832, _ = 0.2368, b = 1.208, R = 1.4142, k = 120, z = 0.5 - 0.5 * i, I = f.sqrt(f.log(1 / (z * z))), E = I + ((((I * c + a) * I + v) * I + s) * I + e) / ((((I * g + t) * I + m) * I + l) * I + o);
          n < k && (E += (E * E * E + E) / n / 4);
          var T = y - _ * E;
          return n < k && (T += -1.214 / n + b * E / n), E * (T * f.log(r - 1) + R);
        }
        u.extend(u.tukey, {
          cdf: function(r, n, e) {
            var o = 1, s = n, l = 16, v = 8, m = -30, a = 1e-14, t = 100, c = 800, g = 5e3, y = 25e3, _ = 1, b = 0.5, R = 0.25, k = 0.125, z = [
              0.9894009349916499,
              0.9445750230732326,
              0.8656312023878318,
              0.755404408355003,
            ...
            if (e > y)
              return w(r, o, s);
            var E = e * 0.5, T = E * f.log(e) - e * f.log(2) - u.gammaln(E), H = E - 1, Q = e * 0.25, x;
            e <= t ? x = _ : e <= c ? x = b : e <= g ? x = R : x = k, T += f.log(x);
            for (var G = 0, A = 1; A <= 50; A++) {
              for (var L = 0, O = (2 * A - 1) * x, V = 1; V <= l; V++) {
                var P, N;
                v < V ? (P = V - v - 1, N = T + H * f.log(O + z[P] * x) - (z[P] * x + O) * Q) : (P = V - 1, N = T + H * f.log(O - z[P] * x) + (z[P] * x - O) * Q);
                var J;
                if (N >= m) {
                  v < V ? J = r * f.sqrt((z[P] * x + O) * 0.5) : J = r * f.sqrt((-(z[P] * x) + O) * 0.5);
                  var C = w(J, o, s), U = C * I[P] * f.exp(N);
                  L += U;
                }
              }
              if (A * x >= 1 && L <= a)
        ...
        u.extend({
          ...
          SOR: function(i, r, n, e, o) {
            for (var s = 0, l = i.length, v = [], m = [], a = [], t, c, g, y, _; s < l; s++)
            ...
            for (y = u.multiply(
              ...
              a,
              u.multiply(v, o)
            )), r), o), c = n, _ = u.add(u.multiply(y, n), g), s = 2; f.abs(u.norm(u.subtract(_, c))) > e; )
              c = _, _ = u.add(u.multiply(y, c), g), s++;
            return _;
          },
          ...
          richardson: function(i, r, n, e) {
            function o(_, b) {
              for (var R = 0, k = _.length, z; R < k; R++)
                _[R] === b && (z = R);
              return z;
            }
          ...
          PCA: function(i) {
            var r = i.length, n = i[0].length, e = 0, o, s, l = [], v = [], m = [], a = [], t = [], c = [], g = [], y = [], _ = [], b = [];
            for (e = 0; e < r; e++)
              l[e] = u.sum(i[e]) / n;
            for (e = 0; e < n; e++)
              for (g[e] = [], o = 0; o < r; o++)
            ...
            for (m = u.jacobi(y), _ = m[0], v = m[1], b = u.transpose(_), e = 0; e < v.length; e++)
              for (o = e; o < v.length; o++)
                v[e] < v[o] && (s = v[e], v[e] = v[o], v[o] = s, a = b[e], b[e] = b[o], b[o] = a);
