...
import {
  useNavigationMorph,
  morphEase,
} from "@/contexts/NavigationMorphContext";
...
const StatusBadge = ({
  ...
  const config =
    type === "ai"
      ...
      : getQueueFileStatusConfig(status);
  const displayText = formatStatus(status);

  return (
    <div
      className={`inline-flex items-center gap-2 px-2.5 py-1 rounded-md ${config.bg} ${config.border} border`}
    >
...
function QueuePageContent() {
  ...
  const [documentCreators, setDocumentCreators] = useState<
    { id: string; name: string }[]
  >([]);
  ...
  const [emailDialogDocument, setEmailDialogDocument] =
    ...
    try {
      ...
      if (savedFilters) {
        ...
        if (filters.documentTypeFilter)
          setDocumentTypeFilter(filters.documentTypeFilter);
        if (filters.entityTypeFilter)
          setEntityTypeFilter(filters.entityTypeFilter);
        if (filters.createdByIdFilter)
          setCreatedByIdFilter(filters.createdByIdFilter);
  ...
  const handleSendEmailNotification = async () => {
    ...
    try {
      ...
      const response = (result.data as any)?.sendPendingDocumentStatusEmail as
      ...
      if (response?.success) {
        ...
        toast.success("Email notification set up successfully!", {
          description:
            "We'll email you when the document processing is complete.",
        });
      } else {
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Select
                      value={aiStatusFilter}
                      onValueChange={(v) =>
                        setAiStatusFilter(v as AiStatus | "all")
                      }
                    >
                    ...
                    <Select
                      value={documentTypeFilter}
                      onValueChange={(v) =>
                        setDocumentTypeFilter(v as DocumentType | "all")
                      }
                    >
                        ...
                        >
                          <div className="flex items-center truncate">
                            ...
                            <span className="truncate">
                              {createdByIdFilter !== "all"
                                ? userCache.get(createdByIdFilter) ||
                                  "Unknown User"
                                : "All Users"}
                            </span>
                          </div>
                    ...
                    {hasActiveFilters && (
                      <Button
                        ...
                        className="h-10 focus:ring-0 focus:ring-offset-0 focus-visible:ring-0"
                      >
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {documents.map((doc, index) => {
                          const entityConfig = getEntityTypeConfig(
                            doc.entityType
                          );
                          const EntityIcon = entityConfig.icon;
                          const DocTypeIcon = getDocumentTypeIcon(
                            doc.documentType
                          );
                              ...
                              >
                                <TooltipProvider>
                                  <Tooltip>
                                    ...
                                    <TooltipContent>
                                      <p>
                                        {formatExactDateTime(doc.createdAt)}
                                      </p>
                                    </TooltipContent>
                                  </Tooltip>
                                </TooltipProvider>
      ...
      <Dialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to archive {selectedIds.size} document(s)?
            ...
            {emailSent ? (
              <>
                <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground text-sm">
                    We&apos;ll send you an email as soon as your document is
                    ready for review.
                  </p>
                </div>
              ...
              <>
                <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground text-sm">
                    This document is currently being processed. You can safely
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg p-3 w-full">
                  <div className="flex items-center gap-3 max-w-full">
                    ...
                    <div className="text-left min-w-0 flex-1">
                      <p
                        className="font-medium text-sm text-foreground truncate max-w-[280px]"
                        title={emailDialogDocument?.fileName || "Document"}
                      >
