...
const generateMockData = (count = 50) => {
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    insideRep: reps[Math.floor(Math.random() * reps.length)],
    ...
    quoteDate: new Date(Date.now() - Math.random() * 90 * 24 * 60 * 60 * 1000).toISOString(),
    expDate: new Date(Date.now() + Math.random() * 60 * 24 * 60 * 60 * 1000).toISOString(),
    ...
    jobName: `Project ${i}`,
    endUser: `End User ${i}`,
    blanket: Math.random() > 0.5 ? "Yes" : "No",
    lostReason: Math.random() > 0.7 ? "Price" : null,
    status: ["Active", "Pending", "Lost", "Won"][Math.floor(Math.random() * 4)],
    quoteId: `QTE-${1000 + i}`,
    orderId: `ORD-${1000 + i}`,
  }));
};
...
export function QuotePivotGrid() {
  ...
  useEffect(() => {
    if (USE_MOCK_DATA) {
      console.log("[QuotePivotGrid] Using MOCK DATA (GraphQL disabled)");
  ...
  useEffect(() => {
    ...
    console.log("[QuotePivotGrid] Subscription data updated:");
    console.log(JSON.stringify(logData, null, 2));

    if (subscriptionData?.getReport?.length) {
      console.log("[QuotePivotGrid] Sample of first item:", subscriptionData.getReport[0]);
    }
  ...
  const getAdditionalState = useCallback(() => {
    console.log('[QuotePivotGrid] getAdditionalState called - advancedFilters:', advancedFilters);
    console.log('[QuotePivotGrid] getAdditionalState called - dateRange:', { startDate, endDate });
    console.log('[QuotePivotGrid] getAdditionalState called - sorting:', sortingConfig);
    return {
      advancedFilters,
      setAdvancedFilters,
    ...
    try {
      ...
      if (grid?.getColumns && grid?.getSource) {
        try {
          ...
          console.warn(
            "[QuotePivotGrid] Failed to retrieve grid data for CSV:",
            error
          );
        }
      }
      ...
      if (exportRows.length === 0) {
        console.warn("[QuotePivotGrid] No data to export");
        return;
      }
      ...
      const filename = `quote-pivot-${dateStamp}`;
  ...
  const transformedRows = useMemo(() => {
    if (!sourceRows || sourceRows.length === 0) {
      console.log("[QuotePivotGrid] No data available - returning empty array");
      return [];
    }
    ...
    try {
      ...
      const transformedData = validRecords.map((record, index) => {
        const quoteDateValue =
          record.quoteDate && !Number.isNaN(new Date(record.quoteDate).getTime())
            ? new Date(record.quoteDate)
            : null;
        const quoteDateIso = quoteDateValue
          ? quoteDateValue.toISOString()
          : typeof record.quoteDate === "string"
          ? record.quoteDate
          : null;
        const quoteDateDisplay = formatDate(quoteDateValue ?? record.quoteDate, dateFormat);

        const baseRow = {
          id: record.id || `quote_${index}`,
          quoteNumber: record.quoteNumber || record.quoteId || "N/A",
          ...
          insideRep: record.insideRep || "N/A",
          ...
          expDate: formatDate(record.expDate, dateFormat),
          entryDate: formatDate(record.entryDate, dateFormat),
          orderDate: formatDate(record.orderDate, dateFormat),

          // Additional fields
          jobName: record.jobName || "N/A",
          endUser: record.endUser || "N/A",
          blanket: record.blanket || "No",
          lostReason: record.lostReason || "N/A",
          status: record.status || "Pending",
          quoteId: record.quoteId || "N/A",
        ...
        const rowWithYtd = attachYtdFields(baseRow, "quoteDate");

        return {
          ...rowWithYtd,
          quoteDate: quoteDateDisplay,
          quoteDateRaw: quoteDateIso,
        };
      });
      ...
      console.error("[QuotePivotGrid] Error transforming data:", error);
      console.error("[QuotePivotGrid] Error stack:", error.stack);
  ...
  const dataMetrics = useMemo(() => {
    ...
    try {
      ...
      console.error("[QuotePivotGrid] Error calculating metrics:", error);
      return null;
    }
      ...
      try {
        ...
        if (grid?.getColumns && grid?.getSource) {
          try {
            ...
            console.warn(
              "[QuotePivotGrid] Failed to retrieve grid data, falling back to sortedRows:",
              error
            );
          }
        }
  ...
  const exportToExcelWithHeaders = useCallback(async () => {
    await runPivotExcelExport({
      ...
      filenamePrefix: "quote-pivot-with-headers",
      reportTitle: "Quote Pivot Report",
    });
  }, [runPivotExcelExport]);
  ...
  useEffect(() => {
    ...
    setSortingConfig(currentSorting => {
      ...
      if (cleanedSorting.length !== currentSorting.length) {
        console.log('[QuotePivotGrid] Cleaning up sorting config - removed columns no longer in pivot');
  ...
  async function resolveColumnsForExport() {
    ...
    if (grid?.getColumns) {
      try {
        ...
        console.warn(
          "[QuotePivotGrid] Failed to retrieve columns from grid for export:",
          error
        );
      }
    }
  ...
  async function fallbackCSVExport() {
    ...
    if (!exportColumns.length || !sortedRows.length) {
      console.warn("[QuotePivotGrid] CSV fallback skipped: nothing to export.");
      return;
    }
    await exportGridCsv({
      filename: `quote-pivot-${new Date().toISOString().split("T")[0]}`,
      columns: exportColumns,
      rows: sortedRows,
    });
  }
  ...
  useEffect(() => {
    ...
    gridRef.current.getPivotConfig = async () => {
      console.log("[QuotePivotGrid] Getting pivot config from React state:", pivotConfig);
      ...
      if (internalGridRef.current) {
        try {
          ...
          if (pivotPlugin) {
            console.log("[QuotePivotGrid] Found pivot plugin with state:", pivotPlugin);
            console.log("[QuotePivotGrid] Plugin keys:", Object.keys(pivotPlugin));
            console.log("[QuotePivotGrid] Plugin.pivotConfig:", pivotPlugin.pivotConfig);

            // The PivotPlugin stores configuration in pivotConfig property
            const pluginConfig = pivotPlugin.pivotConfig || {};
            console.log("[QuotePivotGrid] Plugin.pivotConfig.rows:", pluginConfig.rows);
            console.log("[QuotePivotGrid] Plugin.pivotConfig.columns:", pluginConfig.columns);
            console.log("[QuotePivotGrid] Plugin.pivotConfig.values:", pluginConfig.values);
            ...
            console.log("[QuotePivotGrid] Actual pivot config from plugin (preserved as-is):", actualConfig);
          ...
          console.warn("[QuotePivotGrid] Could not get pivot plugin state:", error);
        }
      }

      // Fallback to React state if plugin not available
      console.warn("[QuotePivotGrid] Using fallback React state (may be outdated)");
      return {
        ...pivotConfig,
    ...
    gridRef.current.setPivotConfig = (newConfig) => {
      console.log("[QuotePivotGrid] Setting pivot config:", newConfig);
      if (newConfig && typeof newConfig === 'object') {
        ...
        setGridKey(prev => prev + 1);
        console.log("[QuotePivotGrid] Forcing grid re-render with new key");
        return true;
      }
      return false;
    };
    ...
    gridRef.current.setPivotState = (newConfig) => {
      if (newConfig) {
        ...
        // Force grid remount to apply new pivot configuration
        setGridKey(prev => prev + 1);
        console.log("[QuotePivotGrid] Forcing grid re-render with new key (setPivotState)");
        return true;
      }
      return false;
    };
  ...
  useEffect(() => {
    ...
    if (typeof grid.addEventListener !== 'function') {
      console.log('[QuotePivotGrid] Grid does not support addEventListener, skipping event listeners');
      return;
    }
  ...
  useEffect(() => {
    if (apiError) {
      console.error("[QuotePivotGrid] API Error:", apiError);
    }
  }, [apiError]);
  ...
  useEffect(() => {
    if (dataMetrics) {
      console.info("[QuotePivotGrid] Data Metrics:", dataMetrics);
    }
  }, [dataMetrics]);
    ...
    <div className="min-h-screen w-full p-4 space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-blue-900/70 font-medium">
              Detailed quote analysis and commissions
            </p>
          </div>
        </div>
      ...
      <PivotSortingModal
        ...
        onSortingChange={(newSorting) => {
          console.log('[QuotePivotGrid] Applying sorting:', newSorting);
      ...
      <FullScreenModal
        ...
        title="Quote Pivot - Full Screen View"
      >
        <div
          ref={containerRef}
