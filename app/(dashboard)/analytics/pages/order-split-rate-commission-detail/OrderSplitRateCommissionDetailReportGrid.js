...
import { useOrderSplitRateCommissionSubscription } from "@/lib/analytics/hooks/useOrderSplitRateCommissionSubscription";
import React from "react";
import { useApolloClient } from "@apollo/client/react";
import { orderSplitRateCommissionDetailReportColumns } from "./column";
...
export function OrderSplitRateCommissionDetailReportGrid() {
  ...
  const {
    ...
    resetCache,
    hasCachedData,
  } = useOrderSplitRateCommissionSubscription(null, startDate, endDate, filterByDate);
  ...
  const {
    ...
    saveNow,
    reloadFromStorage,
  } = useColumnConfig("columns.order_split_rate_commission_detail.v1", orderSplitRateCommissionDetailReportColumns);
  ...
  React.useEffect(() => {
    ...
    const loadTemplates = async () => {
      try {
        ...
        if (isMounted) {
          console.error("[OrderSplitRateCommissionDetailReportGrid] Failed to load saved configs", error);
        }
      }
    };
  ...
  const saveConfig = React.useCallback(
    async (name, additionalState) => {
      ...
      try {
        ...
        console.error("[OrderSplitRateCommissionDetailReportGrid] Failed to save config", error);
        return null;
      }
    },
    [client, config]
  ...
  const loadConfig = React.useCallback(
    async (id) => {
      ...
      try {
        ...
        console.error("[OrderSplitRateCommissionDetailReportGrid] Failed to apply config", error);
        return null;
      }
      return entry;
    },
  ...
  const deleteConfig = React.useCallback(
    async (id) => {
      try {
        ...
        console.error("[OrderSplitRateCommissionDetailReportGrid] Failed to delete config", error);
        return false;
      }
    },
    [client]
  ...
  const setDefaultConfig = React.useCallback(
    async (nextId) => {
      ...
      if (nextId) {
        if (!nextDefault) {
          console.warn("[OrderSplitRateCommissionDetailReportGrid] Unable to find template to mark as default", nextId);
          return;
        }
      ...
      try {
        ...
        console.error("[OrderSplitRateCommissionDetailReportGrid] Failed to update default config", error);
        throw error;
      }
    },
    [client]
  ...
  const gridRows = React.useMemo(
    () =>
      rows.map((r) => ({
        ...r,
        orderTotalCommissionFmt:
          typeof r.orderTotalCommission === "number"
            ? r.orderTotalCommission.toFixed(2)
            : r.orderTotalCommission,
        commissionTotalFmt:
          typeof r.commissionTotal === "number"
  ...
  const handleExportCsv = React.useCallback(() => {
    exportGridCsv({
      filename: "order-split-rate-commission-detail",
      columns: configuredColumns,
      rows: sortedData,
    });
  }, [configuredColumns, sortedData]);
  ...
  const handleExportXlsx = React.useCallback(() => {
    void exportGridXlsx({
      ...
      columns: configuredColumns,
      rows: sortedData,
      sheetName: "Order Split Rate Comm. Detail",
    });
  }, [configuredColumns, sortedData]);

  const handleExportXlsxWithHeaders = React.useCallback(() => {
    void exportGridXlsxWithHeaders({
      filename: "order-split-rate-commission-detail-with-headers",
      columns: configuredColumns,
      rows: sortedData,
      sheetName: "Order Split Rate Comm. Detail",
      ...
      sorting: sorting,
      reportTitle: "Order Split Rate Commission Detail Report"
    });
  }, [configuredColumns, sortedData, advancedFilters, startDate, endDate, sorting]);
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      console.error("[OrderSplitRateCommissionDetailReportGrid] Error loading pending config:", error);
      sessionStorage.removeItem("pendingConfigLoad");
    }
    ...
    <div className=" px-4 py-4 space-y-8">
      <div className="flex flex-wrap gap-4 items-start">
        ...
        <div className="flex items-start pt-4">
          <FilterPane
            columns={orderSplitRateCommissionDetailReportColumns}
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow p-4 flex  justify-between gap-4 items-center border mb-2">
        <div className="w-full flex gap-4 items-center">
          {/* <h3 className="text-lg font-medium">Order Split Rate Commission Detail </h3> */}
        ...
        <div className="flex flex-nowrap justify-end w-full gap-2 sm:gap-4 items-center">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                type="button"
                className="px-4 py-2 flex gap-2 items-center bg-blue-600 text-white rounded-lg font-medium shadow hover:bg-blue-700 transition disabled:opacity-60"
                disabled={Boolean(error) || initialLoading}
              >
                Export
                <ChevronDown size={16} />
      ...
      <div className="">
        {error ? (
          ...
          <DataGrid
            title="Order Split Rate Commission Detail Report"
            columns={configuredColumns}
            searchable={false}
            data={sortedData}
      ...
      <FullScreenModal
        ...
        title="Order Split Rate Commission Detail - Full Screen View"
      >
