...
import { invoiceDetailReportColumns } from "./column";
...
const REPORT_TYPE = "INVOICE_DETAIL_REPORT";
const TABLE_ID = "invoice-detail";

export function InvoiceDetailReportGrid() {
  ...
  const {
    ...
    saveNow,
    reloadFromStorage,
  } = useColumnConfig("columns.invoice_detail.v1", invoiceDetailReportColumns);
  ...
  React.useEffect(() => {
    ...
    const loadTemplates = async () => {
      try {
        ...
        if (isMounted) {
          console.error("[InvoiceDetailReportGrid] Failed to load saved configs", error);
        }
      }
    };
  ...
  const saveConfig = React.useCallback(
    async (name, additionalState) => {
      ...
      try {
        ...
        console.error("[InvoiceDetailReportGrid] Failed to save config", error);
        return null;
      }
    },
    [client, config]
  ...
  const loadConfig = React.useCallback(
    async (id) => {
      ...
      try {
        ...
        console.error("[InvoiceDetailReportGrid] Failed to apply config", error);
        return null;
      }
      return entry;
    },
  ...
  const deleteConfig = React.useCallback(
    async (id) => {
      try {
        ...
        console.error("[InvoiceDetailReportGrid] Failed to delete config", error);
        return false;
      }
    },
    [client]
  ...
  const setDefaultConfig = React.useCallback(
    async (nextId) => {
      ...
      if (nextId) {
        if (!nextDefault) {
          console.warn("[InvoiceDetailReportGrid] Unable to find template to mark as default", nextId);
          return;
        }
      ...
      try {
        ...
        console.error("[InvoiceDetailReportGrid] Failed to update default config", error);
        throw error;
      }
    },
    [client]
      ...
      rows.map((r) => ({
        ...
        unitPriceFmt:
          typeof r.unitPrice === "number"
            ...
            : r.unitPrice,
        quantityOrderedFmt:
          typeof r.quantityOrdered === "number"
            ? r.quantityOrdered.toString()
            : r.quantityOrdered,
        quantityShippedFmt:
          typeof r.quantityShipped === "number"
            ? r.quantityShipped.toString()
            : r.quantityShipped,
        commissionFmt:
  ...
  const handleExportXlsxWithHeaders = React.useCallback(() => {
    void exportGridXlsxWithHeaders({
      filename: "invoice-detail-with-headers",
      columns: columnsWithTemplates,
      rows: sortedData,
      sheetName: "Invoice Detail",
      ...
      sorting: sorting,
      reportTitle: "Invoice Detail Report"
    });
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      console.error("[InvoiceDetailReportGrid] Error loading pending config:", error);
      sessionStorage.removeItem("pendingConfigLoad");
    }
    ...
    <div className=" px-4 py-4 space-y-8">
      <div className="flex flex-wrap gap-4 items-start">
        ...
        <div className="flex items-start pt-4">
          <FilterPane
            columns={invoiceDetailReportColumns}
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow p-4 flex  justify-between gap-4 items-center border mb-2">
        <div className="w-full flex gap-4 items-center">
          {/* <h3 className="text-lg font-medium">Invoice Detail </h3> */}
        ...
        <div className="flex flex-nowrap justify-end w-full gap-2 sm:gap-4 items-center">
          ...
          <ExpandButton
            ...
            disabled={filteredGridRows.length === 0}
          />
      ...
      <FullScreenModal
        ...
        title="Invoice Detail Report - Full Screen View"
      >
