...
import { orderDetailReportColumns } from "./column";
...
const REPORT_TYPE = "ORDER_DETAIL_REPORT";
const TABLE_ID = "orders-detail";

export function OrderDetailReportGrid() {
  ...
  const {
    ...
    saveNow,
    reloadFromStorage,
  } = useColumnConfig("columns.orders_detail.v1", orderDetailReportColumns);
  ...
  React.useEffect(() => {
    ...
    const loadTemplates = async () => {
      try {
        ...
        if (isMounted) {
          console.error("[OrderDetailReportGrid] Failed to load saved configs", error);
        }
      }
    };
  ...
  const saveConfig = React.useCallback(
    async (name, additionalState) => {
      ...
      try {
        ...
        console.error("[OrderDetailReportGrid] Failed to save config", error);
        return null;
      }
    },
    [client, config]
  ...
  const loadConfig = React.useCallback(
    async (id) => {
      ...
      try {
        ...
        console.error("[OrderDetailReportGrid] Failed to apply config", error);
        return null;
      }
      return entry;
    },
  ...
  const deleteConfig = React.useCallback(
    async (id) => {
      try {
        ...
        console.error("[OrderDetailReportGrid] Failed to delete config", error);
        return false;
      }
    },
    [client]
  ...
  const setDefaultConfig = React.useCallback(
    async (nextId) => {
      ...
      if (nextId) {
        if (!nextDefault) {
          console.warn("[OrderDetailReportGrid] Unable to find template to mark as default", nextId);
          return;
        }
      ...
      try {
        ...
        console.error("[OrderDetailReportGrid] Failed to update default config", error);
        throw error;
      }
    },
    [client]
    ...
    return rows.map((r) => {
      ...
      if (r.dueDate) {
        ...
        if (!Number.isNaN(dueDate.getTime())) {
          ...
          daysUntilDueDate = Math.round((dueDate.getTime() - today.getTime()) / msInDay);
        }
      }
      ...
      return {
        ...
        commissionFmt:
          typeof r.commission === "number"
            ...
            : r.commission,
        commissionDiscountFmt:
          typeof r.commissionDiscount === "number"
            ? `${r.commissionDiscount.toFixed(2)}%`
            : r.commissionDiscount,
  ...
  const handleExportXlsxWithHeaders = React.useCallback(() => {
    void exportGridXlsxWithHeaders({
      filename: "orders-detail-with-headers",
      columns: columnsWithTemplates,
      rows: sortedData,
      sheetName: "Orders Detail",
      ...
      sorting: sorting,
      reportTitle: "Orders Detail Report"
    });
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      console.error("[OrderDetailReportGrid] Error loading pending config:", error);
      sessionStorage.removeItem("pendingConfigLoad");
    }
    ...
    <div className=" px-4 py-4 space-y-8">
      <div className="flex flex-wrap gap-4 items-start">
        ...
        <div className="flex items-start pt-4">
          <FilterPane
            columns={orderDetailReportColumns}
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow p-4 flex  justify-between gap-4 items-center border mb-2">
        <div className="w-full flex gap-4 items-center">
          {/* <h3 className="text-lg font-medium">Orders Detail</h3> */}
      ...
      <FullScreenModal
        ...
        title="Order Detail Report - Full Screen View"
      >
