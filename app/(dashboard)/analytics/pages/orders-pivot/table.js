...
import { useOrderDetailSubscription } from "@/lib/analytics/hooks/useOrderDetailSubscription";
...
const generateMockData = (count = 50) => {
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    factoryId: `FACT-${Math.floor(Math.random() * 10)}`,
    outsideRep: reps[Math.floor(Math.random() * reps.length)],
    outsideRepEmail: `rep${i}@example.com`,
    category: categories[Math.floor(Math.random() * categories.length)],
    itemNumber: items[Math.floor(Math.random() * items.length)],
    factoryPartNumber: `FPN-${1000 + i}`,
    customerPartNumber: `CPN-${1000 + i}`,
    detailTotal: (Math.random() * 10000 + 1000).toFixed(2),
    ...
    commissionDiscount: Math.floor(Math.random() * 10),
...
export function OrderPivotGrid() {
  ...
  const {
    ...
    resetCache,
    hasCachedData,
  } = useOrderDetailSubscription(null, startDate, endDate, filterByDate);
  ...
  useEffect(() => {
    if (USE_MOCK_DATA) {
      console.log("[OrderPivotGrid] Using MOCK DATA (GraphQL disabled)");
  ...
  useEffect(() => {
    ...
    console.log("[OrderPivotGrid] Subscription data updated:");
    console.log(JSON.stringify(logData, null, 2));

    if (subscriptionData?.getReport?.length) {
      console.log("[OrderPivotGrid] Sample of first item:", subscriptionData.getReport[0]);
    }
  ...
  const getAdditionalState = useCallback(() => {
    console.log('[OrderPivotGrid] getAdditionalState called - advancedFilters:', advancedFilters);
    console.log('[OrderPivotGrid] getAdditionalState called - dateRange:', { startDate, endDate });
    console.log('[OrderPivotGrid] getAdditionalState called - sorting:', sortingConfig);
    return {
      advancedFilters,
      setAdvancedFilters,
  ...
  useEffect(() => {
    ...
    const handlePendingLoad = (pendingData) => {
      ...
      const isMatch = (pendingData.tableId === "orders-pivot") || (pendingData.reportType === "PIVOT_ORDER_REPORT");
      if (isMatch) {
    ...
    try {
      ...
      if (grid?.getColumns && grid?.getSource) {
        try {
          ...
          console.warn(
            "[OrderPivotGrid] Failed to retrieve grid data for CSV:",
            error
          );
        }
      }
      ...
      if (exportRows.length === 0) {
        console.warn("[OrderPivotGrid] No data to export");
        return;
      }
      ...
      const filename = `orders-pivot-${dateStamp}`;
  ...
  const transformedRows = useMemo(() => {
    if (!sourceRows || sourceRows.length === 0) {
      console.log("[OrderPivotGrid] No data available - returning empty array");
      return [];
    }
    ...
    try {
      ...
      const transformedData = validRecords.map((record, index) => {
        const orderDateValue =
          record.orderDate && !Number.isNaN(new Date(record.orderDate).getTime())
            ? new Date(record.orderDate)
            : null;
        const orderDateIso = orderDateValue
          ? orderDateValue.toISOString()
          : typeof record.orderDate === "string"
          ? record.orderDate
          : null;
        const orderDateDisplay = formatDate(orderDateValue ?? record.orderDate, dateFormat);
        ...
        if (dueDateValue) {
          const normalizedDueDate = new Date(dueDateValue);
          normalizedDueDate.setHours(0, 0, 0, 0);
          daysUntilDueDate = Math.round(
            (normalizedDueDate.getTime() - todayTime) / msInDay
          );
        }

        const baseRow = {
          id: record.id || `order_${index}`,
          orderNumber: record.orderNumber || record.factSoNumber || "N/A",
          customer: String(record.customer || "Unknown Customer"),
          ...
          outsideRepEmail: record.outsideRepEmail,
          category: record.category || "General",
          itemNumber: record.itemNumber || "N/A",
          factoryPartNumber: record.factoryPartNumber || "N/A",
          customerPartNumber: record.customerPartNumber || "N/A",

          // Financial metrics - ensure proper number conversion
          revenue: parseFloat(record.detailTotal) || 0,
          detailTotal: parseFloat(record.detailTotal) || 0,
          ...
          // Ensure outsideRepTotalPortion is populated from API (numeric)
          ...
          commissionDiscount: parseFloat(record.commissionDiscount) || 0,
          outsideRepSplitRate: parseFloat(record.outsideRepSplitRate) || 0,

          // Dates (orderDate kept raw for YTD calculations; display restored later)
          ...
          profitMargin:
            record.detailTotal && record.unitPrice
              ? (
                  ((parseFloat(record.detailTotal) -
          ...
          // Date-based grouping fields extracted from orderDate
          year: extractYear(orderDateValue),
          quarter: extractQuarter(orderDateValue),
          month: extractMonth(orderDateValue, true), // Use month names
        };
        ...
        const rowWithYtd = attachYtdFields(baseRow, "orderDate");

        return {
          ...rowWithYtd,
          orderDate: orderDateDisplay,
          orderDateRaw: orderDateIso,
          dueDateRaw: dueDateIso,
        };
      });
      ...
      console.error("[OrderPivotGrid] Error transforming data:", error);
      console.error("[OrderPivotGrid] Error stack:", error.stack);
  ...
  // Apply sorting with pivot awareness so aggregated value columns respect totals
  ...
  const dataMetrics = useMemo(() => {
    ...
    try {
      ...
      console.error("[OrderPivotGrid] Error calculating metrics:", error);
      return null;
    }
  ...
  const totalsRow = useMemo(() => {
    ...
    const currencyFields = ["revenue", "commission", "unitPrice"];
      ...
      try {
        ...
        if (grid?.getColumns && grid?.getSource) {
          try {
            ...
            console.warn(
              "[OrderPivotGrid] Failed to retrieve grid data, falling back to sortedRows:",
              error
            );
          }
        }
  ...
  const exportToExcelWithHeaders = useCallback(async () => {
    await runPivotExcelExport({
      ...
      filenamePrefix: "orders-pivot-with-headers",
      reportTitle: "Orders Pivot Report",
    });
  }, [runPivotExcelExport]);
  ...
  useEffect(() => {
    ...
    setSortingConfig(currentSorting => {
      ...
      if (cleanedSorting.length !== currentSorting.length) {
        console.log('[OrderPivotGrid] Cleaning up sorting config - removed columns no longer in pivot');
  ...
  const config = useMemo(
    () => ({
      dimensions: [
        ...
        {
          prop: "endUserBillingAddressLineOne",
          name: "End User Billing Address Line 1",
          sortable: true,
          size: 220,
          minSize: 180,
        },
        {
          prop: "endUserBillingAddressLineTwo",
          name: "End User Billing Address Line 2",
          sortable: true,
          size: 220,
          minSize: 180,
        },
        {
          prop: "endUserBillingCity",
          name: "End User Billing City",
          sortable: true,
          size: 180,
          minSize: 140,
        },
        {
          prop: "endUserBillingState",
          name: "End User Billing State",
          sortable: true,
          size: 180,
          minSize: 140,
        },
        {
          prop: "endUserBillingZip",
          name: "End User Billing ZIP",
          sortable: true,
          size: 160,
          minSize: 120,
        },
  ...
  async function resolveColumnsForExport() {
    ...
    if (grid?.getColumns) {
      try {
        ...
        console.warn(
          "[OrderPivotGrid] Failed to retrieve columns from grid for export:",
          error
        );
      }
    }
  ...
  async function fallbackCSVExport() {
    ...
    if (!exportColumns.length || !sortedRows.length) {
      console.warn("[OrderPivotGrid] CSV fallback skipped: nothing to export.");
      return;
    }
    await exportGridCsv({
      filename: `orders-pivot-${new Date().toISOString().split("T")[0]}`,
      columns: exportColumns,
      rows: sortedRows,
    });
  }
  ...
  useEffect(() => {
    ...
    gridRef.current.getPivotConfig = async () => {
      console.log("[OrderPivotGrid] Getting pivot config from React state:", pivotConfig);
      ...
      if (internalGridRef.current) {
        try {
          ...
          if (pivotPlugin) {
            console.log("[OrderPivotGrid] Found pivot plugin with state:", pivotPlugin);
            console.log("[OrderPivotGrid] Plugin keys:", Object.keys(pivotPlugin));
            console.log("[OrderPivotGrid] Plugin.pivotConfig:", pivotPlugin.pivotConfig);

            // The PivotPlugin stores configuration in pivotConfig property
            const pluginConfig = pivotPlugin.pivotConfig || {};
            console.log("[OrderPivotGrid] Plugin.pivotConfig.rows:", pluginConfig.rows);
            console.log("[OrderPivotGrid] Plugin.pivotConfig.columns:", pluginConfig.columns);
            console.log("[OrderPivotGrid] Plugin.pivotConfig.values:", pluginConfig.values);
            ...
            console.log("[OrderPivotGrid] Actual pivot config from plugin (preserved as-is):", actualConfig);
          ...
          console.warn("[OrderPivotGrid] Could not get pivot plugin state:", error);
        }
      }

      // Fallback to React state if plugin not available
      console.warn("[OrderPivotGrid] Using fallback React state (may be outdated)");
      return {
        ...pivotConfig,
    ...
    gridRef.current.setPivotConfig = (newConfig) => {
      console.log("[OrderPivotGrid] Setting pivot config:", newConfig);
      if (newConfig && typeof newConfig === 'object') {
        ...
        setGridKey(prev => prev + 1);
        console.log("[OrderPivotGrid] Forcing grid re-render with new key");
        return true;
      }
      return false;
    };
    ...
    gridRef.current.setPivotState = (newConfig) => {
      if (newConfig) {
        ...
        // Force grid remount to apply new pivot configuration
        setGridKey(prev => prev + 1);
        console.log("[OrderPivotGrid] Forcing grid re-render with new key (setPivotState)");
        return true;
      }
      return false;
    };
  ...
  useEffect(() => {
    ...
    if (typeof grid.addEventListener !== 'function') {
      console.log('[OrderPivotGrid] Grid does not support addEventListener, skipping event listeners');
      return;
    }
  ...
  useEffect(() => {
    if (apiError) {
      console.error("[OrderPivotGrid] API Error:", apiError);
    }
  }, [apiError]);
  ...
  useEffect(() => {
    if (dataMetrics) {
      console.info("[OrderPivotGrid] Data Metrics:", dataMetrics);
    }
  }, [dataMetrics]);
  ...
  return (
    <div className="min-h-screen w-full p-4 space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-blue-900/70 font-medium">
              Detailed order lines and commissions
            </p>
          </div>
        </div>
      ...
      <FullScreenModal
        ...
        title="Orders Pivot - Full Screen View"
      >
        <div
