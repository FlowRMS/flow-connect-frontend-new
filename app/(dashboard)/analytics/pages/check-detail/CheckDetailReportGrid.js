...
import { checkDetailReportColumns } from "./column";
...
const REPORT_TYPE = "CHECK_DETAIL_REPORT";
const TABLE_ID = "check-detail";

export function CheckDetailReportGrid() {
  // Date range state - Applied values (actually used for API calls)
  ...
  const {
    ...
    saveNow,
    reloadFromStorage,
  } = useColumnConfig("columns.check_detail.v1", checkDetailReportColumns);
  ...
  React.useEffect(() => {
    ...
    const loadTemplates = async () => {
      try {
        ...
        if (isMounted) {
          console.error("[CheckDetailReportGrid] Failed to load saved configs", error);
        }
      }
    };
  ...
  const saveConfig = React.useCallback(
    async (name, additionalState) => {
      ...
      try {
        ...
        console.error("[CheckDetailReportGrid] Failed to save config", error);
        return null;
      }
    },
    [client, config]
  ...
  const loadConfig = React.useCallback(
    async (id) => {
      ...
      try {
        ...
        console.error("[CheckDetailReportGrid] Failed to apply config", error);
        return null;
      }
      return entry;
    },
  ...
  const deleteConfig = React.useCallback(
    async (id) => {
      try {
        ...
        console.error("[CheckDetailReportGrid] Failed to delete config", error);
        return false;
      }
    },
    [client]
  ...
  const setDefaultConfig = React.useCallback(
    async (nextId) => {
      ...
      if (nextId) {
        if (!nextDefault) {
          console.warn("[CheckDetailReportGrid] Unable to find template to mark as default", nextId);
          return;
        }
      ...
      try {
        ...
        console.error("[CheckDetailReportGrid] Failed to update default config", error);
        throw error;
      }
    },
    [client]
  ...
  const gridRows = React.useMemo(
    () =>
      rows.map((r) => ({
        ...
        paidCommissionFmt:
          typeof r.paidCommission === "number"
            ? r.paidCommission.toFixed(2)
            : r.paidCommission,
      })),
    [rows]
  );
  ...
  const handleExportXlsxWithHeaders = React.useCallback(() => {
    void exportGridXlsxWithHeaders({
      ...
      sorting: sorting,
      reportTitle: "Check Detail Report"
    });
    ...
    <div className=" px-4 py-4 space-y-8">
      <div className="flex flex-wrap gap-4 items-start">
        ...
        <div className="flex items-start pt-4">
          <FilterPane
            columns={checkDetailReportColumns}
      ...
      {(Object.keys(advancedFilters).length > 0 || startDate || endDate) && (
        <ActiveFilters
          ...
          filterByDate={startDate || endDate ? filterByDate : null}
          onClearDateRange={() => {
            setStartDate("");
            setEndDate("");
            setFilterByDate("ENTITY_DATE");
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow p-4 flex  justify-between gap-4 items-center border mb-2">
        <div className="w-full flex gap-4 items-center">
          {/* <h3 className="text-lg font-medium">Check Detail</h3> */}
      ...
      <FullScreenModal
        ...
        title="Check Detail Report - Full Screen View"
      >
