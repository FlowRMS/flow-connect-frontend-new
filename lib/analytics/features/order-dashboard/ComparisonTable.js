...
import { useGlobalDashFilters } from './GlobalDashFilterContext';
...
export const ComparisonTable = ({
  ...
  const { state: globalFilters } = useGlobalDashFilters();
...
const varianceCell = (h, { value }) => {
  ...
  // Build active filters for tooltip
  const getActiveFilters = () => {
    const filters = [];
    
    if (globalFilters.customers?.length > 0) {
      filters.push({
        label: 'Customers',
        values: globalFilters.customers.map(c => c.label)
      });
    }
    
    if (globalFilters.factories?.length > 0) {
      filters.push({
        label: 'Factories',
        values: globalFilters.factories.map(f => f.label)
      });
    }
    
    if (globalFilters.outsideReps?.length > 0) {
      filters.push({
        label: 'Outside Reps',
        values: globalFilters.outsideReps.map(r => r.label)
      });
    }
    
    if (globalFilters.factoryPartNumbers?.length > 0) {
      filters.push({
        label: 'Factory Part Numbers',
        values: globalFilters.factoryPartNumbers.map(p => p.label)
      });
    }
          ...
          <div className="absolute right-4 top-4 z-10 flex items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center justify-center w-8 h-8 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-full transition-colors"
              ...
              {showTooltip && (
                <div className="absolute right-0 top-10 w-96 bg-white border border-gray-200 rounded-lg shadow-xl p-4 z-50 max-h-[500px] overflow-y-auto">
                  <div className="space-y-3 text-sm">
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-2">Data Information</h4>
                      <ul className="space-y-1 text-gray-700 ml-2">
                        <li className="flex items-start">
                          <span className="mr-2">•</span>
                          <span>Year-over-year comparison (This Year vs Last Year)</span>
                        </li>
                        <li className="flex items-start">
                          <span className="mr-2">•</span>
                          <span>May exclude cancelled order lines</span>
                        </li>
                        <li className="flex items-start">
                          <span className="mr-2">•</span>
                          <span>Use Orders Pivot for detailed line-item analysis</span>
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="pt-3 border-t border-gray-200">
                      ...
                      {getActiveFilters().length > 0 ? (
                        ...
                        <p className="text-gray-500 italic text-xs">No filters applied</p>
                      )}
                    </div>
                  </div>
                </div>
