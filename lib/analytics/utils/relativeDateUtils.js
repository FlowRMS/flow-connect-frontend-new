...
export function detectRelativePeriod(startDateStr, endDateStr) {
  ...
  for (const option of RELATIVE_DATE_OPTIONS) {
    ...
    if (["today", "last7days", "last30days", "thisMonth", "thisQuarter", "thisYear", "last2Years"].includes(option.value)) {
...
/**
 * Get the default 2-year date range for pivot/detail pages
 * @returns {Object} Object with startDate and endDate in yyyy-MM-dd format
 */
export function getDefault2YearRange() {
  const now = new Date();
  return {
    startDate: format(subYears(startOfDay(now), 2), "yyyy-MM-dd"),
