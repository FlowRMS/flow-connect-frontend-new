...
export function ContactSettingsTab() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (success) {
        showSuccessToast('Contact settings saved to Tenant Settings');
        setPicklistHasChanges(false);
      } else {
        showErrorToast('Failed to save contact settings');
      }
    } catch (error) {
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="sticky top-0 z-10 bg-[var(--background)] pb-4 border-b border-[var(--border)]">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-[var(--foreground)]">Contact Settings</h2>
            <p className="text-sm text-[var(--muted-foreground)] mt-1">
              Configure default settings for contacts
            </p>
          </div>
      ...
      {/* Contact Roles Picklist - Only visible for tenant settings */}
      {scope === 'tenant' && (
      ...
      {/* Message when viewing My Settings */}
      {scope === 'my' && (
        <div className="bg-[var(--card)] rounded-lg border border-[var(--border)] p-6">
          <p className="text-sm text-[var(--muted-foreground)]">
            Contact role customization is only available in Tenant Settings.
            Switch to Tenant Settings to manage contact roles.
          </p>
        </div>
      )}
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-blue-600 mt-1">
              Contact role settings are applied tenant-wide and affect all users.
              Changes to roles will be reflected in contact creation, editing, and filtering.
            </p>
          </div>
        </div>
      </div>
