      ...
      <div className="overflow-x-auto">
        <div className="min-w-[900px]">
          ...
          <div className="grid grid-cols-12 gap-2 md:gap-4 px-4 md:px-6 py-3 md:py-4 border-b border-[var(--border)] bg-gray-50/80">
            <div className="col-span-4 text-[10px] md:text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider">
              Project Details
            </div>
            ...
            <div className="col-span-2 text-[10px] md:text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider text-right">
              Actions
            </div>
          </div>
            ...
            {takeoffs.map((takeoff) => {
              ...
              return (
                <div
                  ...
                  className="grid grid-cols-12 gap-2 md:gap-4 px-4 md:px-6 py-3 md:py-4 hover:bg-blue-50/50 transition-colors cursor-pointer group"
                >
                  {/* Project Details */}
                  <div className="col-span-4 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                      ...
                      {docCounts.fixtureSchedules > 0 && (
                        <span className="inline-flex items-center px-2 py-0.5 bg-purple-100 text-purple-700 rounded text-[10px] font-medium">
                          Fixture Schedules
                        </span>
                      )}
                      {docCounts.specifications > 0 && (
                        <span className="inline-flex items-center px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-[10px] font-medium">
                          Specifications
                        </span>
                      )}
                      {docCounts.blueprints > 0 && (
                        <span className="inline-flex items-center px-2 py-0.5 bg-green-100 text-green-700 rounded text-[10px] font-medium">
                          Blueprints
                        </span>
                      )}
                    </div>
                  ...
                  {/* Status */}
                  <div className="col-span-1 flex items-center">
                    <span className={`px-1.5 md:px-2.5 py-0.5 md:py-1 rounded-full text-[10px] md:text-xs font-medium whitespace-nowrap ${getStatusColor(takeoff.status)}`}>
                      {takeoff.status}
                    </span>
                  </div>
                  ...
                  <div className="col-span-2 flex items-center justify-end gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2 md:px-3 py-1 md:py-1.5 text-[10px] md:text-xs font-medium text-[var(--foreground)] hover:bg-[var(--muted)] rounded-md transition-colors"
                    >
                    ...
                    {onDeleteTakeoff && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-2 md:px-3 py-1 md:py-1.5 text-[10px] md:text-xs font-medium text-red-600 hover:bg-red-50 rounded-md transition-colors"
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    {takeoff.quoteId && onViewQuote && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-2 md:px-3 py-1 md:py-1.5 text-[10px] md:text-xs font-medium text-blue-600 hover:bg-blue-50 rounded-md transition-colors"
                      >
                        Quote
                      </button>
                    )}
