/**
 * Checks API Module
 * GraphQL queries and mutations for commission checks
 */
...
export function useCheck(checkId: string | null) {
  return useQuery({
    ...
    queryFn: () => (checkId ? fetchCheckById(checkId) : null),
    enabled: !!checkId,
  });
}
...
export function useChecksLandingPage(
  ...
  return useQuery({
    ...
    queryFn: () => fetchChecksLandingPage(filters, limit),
  });
}
...
export function useCheckSearch(searchTerm: string, limit: number = 50) {
  return useQuery({
    ...
    queryFn: () => searchChecks(searchTerm, limit),
  });
}
...
export function useChecksByFactory(factoryId: string | null) {
  return useQuery({
    ...
    queryFn: () => (factoryId ? fetchChecksByFactory(factoryId) : []),
    enabled: !!factoryId,
  });
}
...
export function useCreateCheck() {
  ...
  return useMutation({
    mutationFn: (input: CreateCheckInput) => createCheck(input),
    onSuccess: () => {
...
export function useUpdateCheck() {
  ...
  return useMutation({
    mutationFn: (input: UpdateCheckInput) => updateCheck(input),
    onSuccess: (data) => {
      // Invalidate and refetch
