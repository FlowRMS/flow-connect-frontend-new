...
type CustomerFilterProps = {
  option: FilterOption;
  selectedValues: string[]; // Array of customer names
...
export function CustomerFilter({
  ...
  const { data: customers = [], isLoading, isError } = useQuery<CustomerSearchResult[], Error>({
    queryKey: ['customerSearch-filter', searchTermForQuery],
    queryFn: () => searchCustomers(searchTermForQuery, true, searchTermForQuery ? 50 : 20),
        ...
        {isLoading && filteredCustomers.length === 0 ? (
          ...
          filteredCustomers.map((customer) => (
            ...
            >
              <input
                type="checkbox"
                checked={selectedValues.includes(customer.companyName)}
                onChange={() => onToggleValue(customer.companyName)}
