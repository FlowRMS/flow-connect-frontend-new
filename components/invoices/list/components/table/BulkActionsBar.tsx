 ...
 * Displays when invoices are selected, shows count and bulk actions menu
 */
...
interface BulkActionsBarProps {
  ...
  onBulkSetStatus: (status: InvoiceStatus) => void;
  onBulkDelete: () => void;
}
  ...
  return (
    <div className="px-4 py-2 bg-[var(--primary)]/5 border-b border-[var(--border)] flex items-center justify-between">
      <span className="text-sm text-[var(--foreground)]">
        <strong>{selectedCount}</strong> invoice{selectedCount !== 1 ? 's' : ''}{' '}
        selected
      </span>
                ...
                <div className="relative group">
                  ...
                  <div className="absolute left-full top-0 ml-1 w-48 bg-white border border-[var(--border)] rounded-lg shadow-xl py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                    {invoiceStatuses.map((status) => (
                      ...
                      >
                        <span
                          className={`inline-block px-2 py-0.5 text-xs font-medium rounded ${invoiceStatusColors[status]}`}
                        >
