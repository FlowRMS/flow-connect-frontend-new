...
export default function MarketTrackPivot() {
  ...
  const fetchPivotData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/disc/market-track/pivot?${params}`);
  ...
  }, [selectedRows, selectedColumns, selectedValues, selectedSection, selectedCategory, selectedYear]);
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col p-6 bg-[var(--background)]">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-bold text-[var(--foreground)]">Market Track Pivot</h1>
          <p className="text-[var(--muted-foreground)] mt-1">Create pivot table analysis of Market Track data</p>
        </div>
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-lg p-4 mb-4">
          <div className="flex items-center gap-4 flex-wrap">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-[var(--muted-foreground)]">Section</label>
              <select
                value={selectedSection}
                onChange={(e) => setSelectedSection(e.target.value)}
                className="px-3 py-2 bg-[var(--input)] border border-[var(--input-border)] rounded-lg text-sm min-w-[140px]"
              >
          ...
          {loading ? (
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center text-[var(--muted-foreground)]">
                ...
                <h3 className="text-lg font-medium text-[var(--foreground)] mb-2">Market Track Pivot Table</h3>
                      ...
                      {columnValues.length > 0 ? (
                        columnValues.map(colVal => (
                          <td key={colVal} className="px-3 py-2 text-sm text-right text-[var(--foreground)] border border-[var(--border)]">
                            {typeof row[colVal] === 'number' ? row[colVal].toLocaleString() : String(row[colVal] ?? '-')}
                          </td>
                        ))
                      ) : selectedValues.length > 0 ? (
                        <td className="px-3 py-2 text-sm text-right text-[var(--foreground)] border border-[var(--border)]">
                          {typeof row['total_value'] === 'number' ? row['total_value'].toLocaleString() : '-'}
                        </td>
                      ) : null}
                    </tr>
                  ))}
