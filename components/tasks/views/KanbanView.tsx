  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 overflow-x-auto">
      {TASK_STAGES.map((stage) => (
        <div
          key={stage.name}
          className="bg-[var(--card)] border border-[var(--border)] rounded-lg min-w-[250px] sm:min-w-0"
          onDragOver={handleDragOver}
          onDrop={(e) => handleDrop(e, stage)}
        >
          <div className="p-3 md:p-4 border-b border-[var(--border)]">
            <h3 className="font-semibold text-[var(--foreground)] flex items-center justify-between text-sm md:text-base">
              <span className="truncate">{stage.label}</span>
              <span className="text-xs text-[var(--muted-foreground)] bg-[var(--muted)] px-2 py-1 rounded flex-shrink-0 ml-2">
                {getTasksByAPIStatus(stage.name).length}
              </span>
            </h3>
          </div>
          <div className="p-2 space-y-2 min-h-[300px] md:min-h-[500px]">
            {getTasksByAPIStatus(stage.name).map((task) => (
              <div
                ...
                onDragEnd={onDragEnd}
                className={`bg-white border border-[var(--border)] rounded-lg p-2.5 md:p-3 cursor-move hover:shadow-md transition-all ${
                  draggedTaskId === task.id ? 'opacity-50 scale-95' : ''
                }`}
              >
                <div className="flex items-start justify-between mb-2 gap-2">
                  <h4 className="font-medium text-xs md:text-sm text-[var(--foreground)] line-clamp-2 break-words min-w-0">{task.title}</h4>
                  <button
                ...
                <p className="text-[10px] md:text-xs text-[var(--muted-foreground)] mb-2 line-clamp-2 break-words">{task.description}</p>
                <div className="flex flex-col gap-1 text-[10px] md:text-xs text-[var(--muted-foreground)] mb-2">
                ...
                <div className="flex items-center gap-1 flex-wrap">
                  {task.tags.slice(0, 2).map((tag, idx) => (
                    <span
                      key={idx}
                      className="px-1.5 md:px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-[10px] md:text-xs truncate max-w-[80px]"
                    >
                      {tag}
                    </span>
                  ))}
                  {task.tags.length > 2 && (
                    <span className="text-[10px] md:text-xs text-[var(--muted-foreground)]">+{task.tags.length - 2}</span>
                  )}
                </div>
                {/* Linked Entities */}
                {(task.entities?.jobs?.length || task.entities?.contacts?.length || task.entities?.companies?.length) ? (
                  <div className="flex gap-1 flex-wrap mt-2">
                    {task.entities?.jobs?.slice(0, 1).map((job) => (
                      <span key={job.id} className="px-1.5 md:px-2 py-0.5 bg-green-100 text-green-700 rounded text-[10px] md:text-xs flex items-center gap-1 max-w-full">
                    ...
                    {task.entities?.contacts?.slice(0, 1).map((contact) => (
                      <span key={contact.id} className="px-1.5 md:px-2 py-0.5 bg-orange-100 text-orange-700 rounded text-[10px] md:text-xs flex items-center gap-1 max-w-full">
                        <svg className="w-3 h-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    {task.entities?.companies?.slice(0, 1).map((company) => (
                      <span key={company.id} className="px-1.5 md:px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded text-[10px] md:text-xs flex items-center gap-1 max-w-full">
                        <svg className="w-3 h-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    {((task.entities?.jobs?.length ?? 0) + (task.entities?.contacts?.length ?? 0) + (task.entities?.companies?.length ?? 0)) > 3 && (
                      <span className="text-[10px] md:text-xs text-[var(--muted-foreground)]">+more</span>
                    )}
                  </div>
                ) : null}
                <div className="mt-2 pt-2 border-t border-[var(--border)] flex items-center justify-between gap-2">
                  <span className="text-[10px] md:text-xs text-[var(--muted-foreground)] truncate">{task.assignedTo}</span>
                  {(task.apiPriority === 'URGENT' || task.apiPriority === 'CRITICAL') && (
                    <span className={`text-[10px] md:text-xs font-medium flex-shrink-0 ${task.apiPriority === 'CRITICAL' ? 'text-purple-600' : 'text-red-500'}`}>
