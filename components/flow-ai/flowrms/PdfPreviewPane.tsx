...
export function PdfPreviewPane({ file, fileUrl, fileName, pages, onMarkdownTextSelect }: PdfPreviewPaneProps) {
  ...
  const [isFullscreenOpen, setIsFullscreenOpen] = useState(false); // Dialog fullscreen state
  ...
  }, [isFullscreenOpen, onMarkdownTextSelect]);
  ...
  const documentSource = useMemo(() => {
    ...
    if (fileUrl) {
      ...
      if (fileUrl.includes('digitaloceanspaces.com') || fileUrl.includes('s3.')) {
        const proxyUrl = `/api/flow-ai/pdf-proxy?url=${encodeURIComponent(fileUrl)}`;
        return { url: proxyUrl };
      }
      return { url: fileUrl };
    }
  ...
  // Render the PDF content - shared between inline and fullscreen
  const renderPdfContent = (isInDialog = false) => (
    <div className={`${isInDialog ? 'h-full' : 'h-full'} overflow-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100`} ref={isInDialog ? undefined : scrollAreaRef}>
              ...
              {numPages > 0 && Array.from(new Array(numPages), (_, index) => (
                <div
                  ...
                  style={{
                    ...
                    width: isInDialog ? '800px' : `${pageWidth}px`
                  }}
                >
                  <Page
                    pageNumber={index + 1}
                    ...
                    width={isInDialog ? 800 : pageWidth}
                    rotate={rotation}
  ...
  return (
    <>
      <Card className={`flow-card h-full flex flex-col ${viewMode === 'markdown' ? '' : 'animated-border'}`}>
                  ...
                  {viewMode === 'markdown' && hasMarkdown ? (
                    <div className="h-full bg-background rounded-lg overflow-hidden">
                      <MarkdownViewer
                        ...
                        closeDialog={() => setIsFullscreenOpen(false)}
                      />
                    </div>
                  ) : (
