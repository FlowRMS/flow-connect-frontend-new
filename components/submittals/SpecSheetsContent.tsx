...
export default function SpecSheetsContent() {
  ...
  // Derive folders from spec sheets based on their folder_path
  const derivedFolders = useMemo(() => {
    ...
    return deriveFoldersFromSpecSheets(specSheets || [], manufacturerName);
  }, [specSheets, selectedManufacturerId, manufacturers]);

  // Folder management state - initialize with derived folders, allow local modifications
  const [localFolderOverrides, setLocalFolderOverrides] = useState<SpecSheetFolder[]>([]);
  const folders = useMemo(() => {
    // Merge derived folders with any local overrides (new folders created by user)
    const merged = [...derivedFolders];
    for (const override of localFolderOverrides) {
      if (!merged.find(f => f.id === override.id)) {
        merged.push(override);
      }
    }
    return merged;
  }, [derivedFolders, localFolderOverrides]);
  ...
  const handleSaveRename = () => {
    if (editingFolderId && editingFolderName.trim()) {
      setLocalFolderOverrides(prev => prev.map(f =>
  ...
  const handleDeleteFolder = (folder: SpecSheetFolder) => {
    if (confirm(`Delete folder "${folder.name}"? This will not delete the spec sheets inside.`)) {
      ...
      setLocalFolderOverrides(prev => prev.filter(f => !idsToDelete.has(f.id)));
  ...
  const handleCreateFolder = () => {
    if (newFolderName.trim()) {
      ...
      setLocalFolderOverrides(prev => [...prev, newFolder]);
